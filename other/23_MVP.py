...
class DataProcessor:
    def __init__(self):
        ...
        self.model = load_model('ResNet_model.h5')
        ...
        for message in self.consumer:
            try:
                ...
                if len(self.hundred_data) >= 100:
                    ...
                    predictions = self.make_prediction(self.hundred_data)
                    for prediction in predictions:
                        print(f"Prediction: {'Normal' if prediction == 0 else 'Anomaly'}")
    ...
    def make_prediction(self, data_batch):
        # Extract the relevant features
        features = [[d['ax'], d['ay'], d['az'], d['gx'], d['gy'], d['gz']] for d in data_batch]
        features = np.array(features)
        
        # Normalize the features using the loaded scaler
        features = self.scaler.transform(features)
        
        # Reshape the features for Conv1D (samples, timesteps, channels)
