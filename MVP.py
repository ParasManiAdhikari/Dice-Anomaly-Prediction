...
class DataProcessor:
    ...
    def perform_prediction(self):
        ...
        while len(self.hundred_data) < self.batchsize:
            if first_run:
                first_run = False
        ...
        responses = []
        input = self.find_peak_data_of_100()
        for data in input:
        ...
        majority_label, count_normal, count_anomalie = self.determine_majority_label(responses)
        return {"prediction": majority_label, "n_count": count_normal, "a_count": count_anomalie}
    
    def find_peak_data_of_100(self):
        return self.hundred_data
