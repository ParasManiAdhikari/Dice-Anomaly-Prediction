...
conn = sqlite3.connect('21/Training_Zustand.db')
query = "SELECT * FROM mockdata WHERE zustand = 1" 
...
summary_statistics.to_csv('summary_statistics_by_label_2.csv', index=False)
...
for label in df['label'].unique():
    ...
    print(f'Correlation matrix for Label: {label}')
    print(corr_matrix)
    plt.figure(figsize=(12, 8))
